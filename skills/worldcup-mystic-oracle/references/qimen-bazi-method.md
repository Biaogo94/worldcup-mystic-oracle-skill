...
Read `qimen-engine.md` before charting. Prefer the configured `Biaogo94/qimen` MCP when available:

```bash
node scripts/qimen_mcp_client.mjs --datetime 2026-06-14T12:00:00-05:00 --location "Houston" --pretty
```

Use the MCP JSON as the primary structured chart. It supplies palace-level 地盘、天盘、八门、九星、八神、暗干、空亡、马星、值符、值使, 格局, and 九宫解断 fields. After charting, read `qimen-scoring.md` and convert the chart into result qi, handicap qi, goal-channel qi, tempo qi, and score-image qi.
